...
def setupDatabase():
    ...
    for city in cities:
        query = db.insert(Cities).values(
            id=city["id"] + 1,
            airbnb_listings=city["airbnb_listings"],
            cost=city["cost"],
            hiking_trails=city["hiking_trails"],
            ...
            nearest_parks=city["nearest_parks"],
            nearest_airports=city["nearest_airports"],
        )
    ...
    for airport in airports:
        query = db.insert(Airports).values(
            id=airport["id"] + 1,
            ...
            name=airport["name"],
            nearest_cities=airport["nearest_cities"],
            nearest_parks=airport["nearest_parks"],
            phone=airport["phone"],
            state=airport["state"],
            website=airport["website"],
            zip_code=airport["zip_code"],
        )
        Result = conn.execute(query)
